<div id="edit-document-modal-{{ $document->id }}" class="hs-overlay hidden fixed inset-0 z-[80] overflow-y-auto overflow-x-hidden">
  <div class="hs-overlay-open:mt-7 mx-3 sm:mx-auto sm:w-full sm:max-w-3xl transition-all ease-out">
    <div class="relative flex flex-col bg-white border border-gray-200 rounded-xl shadow-lg dark:bg-neutral-900 dark:border-neutral-700">
      ...
      <div class="flex items-center justify-between px-4 py-3 border-b dark:border-neutral-700">
        ...
        <button type="button" class="inline-flex items-center justify-center size-8 rounded-full text-gray-500 hover:bg-gray-100 dark:text-neutral-400 dark:hover:bg-neutral-800" data-hs-overlay="#edit-document-modal-{{ $document->id }}">
      ...
      <!-- Form -->
      <form method="POST" action="{{ route('document.update', $document) }}" enctype="multipart/form-data" class="p-4 sm:p-6">
        @csrf
        @method('PUT')
        <div class="space-y-5">
          <div>
          ...
          <div>
            ...
            <textarea name="description" rows="3"
              class="block w-full px-3 py-2 text-sm border border-gray-200 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-200 dark:placeholder-neutral-500">{{ $document->description }}</textarea>
          </div>
          ...
          <div>
            ...
            <select name="category"
              ...
              @foreach($categories as $cat)
                <option value="{{ $cat }}" {{ $document->category === $cat ? 'selected' : '' }}>
                  {{ ucfirst($cat) }}
                </option>
              @endforeach
            </select>
          ...
          <div>
            <label class="block mb-2 text-sm font-medium text-gray-700 dark:text-neutral-200">File (Opsional)</label>
            ...
            @if($document->file_url)
              <p class="mt-2 text-xs text-gray-500 dark:text-neutral-400">
                File saat ini: 
                <a href="{{ $document->file_url }}" target="_blank" class="font-medium text-blue-600 hover:underline dark:text-blue-400">
                  Lihat
                </a>
              </p>
            @endif
        ...
        <div class="flex items-center justify-end gap-x-3 px-4 py-3 mt-6 border-t dark:border-neutral-700">
          <button type="button" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 dark:bg-neutral-900 dark:text-neutral-200 dark:border-neutral-700 dark:hover:bg-neutral-800" data-hs-overlay="#edit-document-modal-{{ $document->id }}">
            Batal
          </button>
