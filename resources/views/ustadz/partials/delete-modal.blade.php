<div id="delete-ustadz-modal-{{ $ustadz->id }}" class="hs-overlay hidden size-full fixed top-0 start-0 z-80 overflow-x-hidden overflow-y-auto backdrop-brightness-50">
  <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-neutral-900 dark:border-neutral-700">
      ...
      <div class="flex justify-between items-center py-3 px-4 border-b dark:border-neutral-700">
        <h3 class="font-bold text-gray-800 dark:text-white">Hapus Data Ustadz</h3>
        <button type="button"
                class="size-7 flex justify-center items-center rounded-full hover:bg-gray-100 dark:hover:bg-neutral-700"
                data-hs-overlay="#delete-ustadz-modal-{{ $ustadz->id }}">
          ✕
        </button>
      </div>
      ...
      <div class="p-4">
        <p class="text-gray-700 dark:text-gray-300">
          Yakin ingin menghapus ustadz <span class="font-semibold">{{ $ustadz->full_name }}</span> (NIP: {{ $ustadz->nip }})?
        </p>
      </div>

      <!-- Footer -->
      <div class="flex justify-end gap-x-2 py-3 px-4 border-t dark:border-neutral-700">
        <button type="button" class="py-2 px-3 rounded-lg border bg-white text-gray-800 hover:bg-gray-50 dark:bg-neutral-900 dark:text-white dark:hover:bg-neutral-800"
                data-hs-overlay="#delete-ustadz-modal-{{ $ustadz->id }}">
          Batal
        </button>
