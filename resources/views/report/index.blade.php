            ...
            <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div>
                    ...
                    <select name="period_type"
                        ...
                        <option value="semester" {{ request('period_type') === 'semester' ? 'selected' : '' }}>Semester
                        </option>
                        <option value="yearly" {{ request('period_type') === 'yearly' ? 'selected' : '' }}>Tahunan
                        </option>
                    </select>
                </div>
                    ...
                    <div
                        ...
                        <div class="p-4 border-b border-gray-200 dark:border-neutral-700">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span
                                    class="text-xs text-gray-500 dark:text-gray-400">{{ ucfirst($report->period_type) }}</span>
                            </div>
                            <h3 class="font-semibold text-gray-900 dark:text-white">{{ $report->santri->full_name }}
                            </h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                {{ $report->santri->tpaClass->display_name }}</p>
                        </div>
                        ...
                        <div class="p-4">
                            <div class="space-y-3">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span
                                        class="text-sm font-medium text-gray-900 dark:text-white">{{ $report->period_label }}</span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Tahun Ajaran:</span>
                                    <span
                                        class="text-sm font-medium text-gray-900 dark:text-white">{{ $report->academic_year }}</span>
                                </div>
                                @if ($report->overall_score)
                                    <div class="flex justify-between items-center">
                                        <span class="text-sm text-gray-600 dark:text-gray-400">Nilai Keseluruhan:</span>
                                        <span
                                            class="text-lg font-bold text-blue-600 dark:text-blue-400">{{ number_format($report->overall_score, 1) }}</span>
                                    </div>
                                @endif
                            </div>
                            ...
                            @if ($report->summary_scores)
                                <div class="mt-4 pt-4 border-t border-gray-200 dark:border-neutral-700">
                                    <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-2">Ringkasan Nilai:
                                    </h4>
                                    <div class="space-y-1">
                                        @foreach ($report->summary_scores as $category => $data)
                                            <div class="flex justify-between items-center text-sm">
                                                <span
                                                    class="text-gray-600 dark:text-gray-400">{{ $data['display_name'] }}:</span>
                                                <span
                                                    class="font-medium text-gray-900 dark:text-white">{{ $data['score'] ? number_format($data['score'], 1) : '-' }}</span>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                            ...
                            <div class="mt-4 flex space-x-2">
                                <button type="button" data-hs-overlay="#show-report-modal-{{ $report->id }}"
                                ...
                                <button type="button" data-hs-overlay="#edit-report-modal-{{ $report->id }}"
                                ...
                                <button type="button" data-hs-overlay="#delete-report-modal-{{ $report->id }}"
